...
def get_google_credentials():
    """Loads Google credentials from environment variables."""
    ...
    try:
        if google_creds_json.strip().startswith('{'):
            ...
            google_credentials = service_account.Credentials.from_service_account_file(google_creds_json)
        return google_credentials
...
def get_speech_client():
    ...
    print("Initializing Google Speech Client for the first time...")
...
def get_tts_client():
    ...
    print("Initializing Google TTS Client for the first time...")
    creds = get_google_credentials()
    if creds:
...
def get_gemini_model():
    ...
    print("Initializing Google Gemini Model for the first time...")
...
ROLEPLAY_CONTEXTS = {
    "school": (
        "You are Genie, an AI English tutor. You're roleplaying as a new classmate with a child. "
        "Your goal is to have a fun and natural conversation about school. Let the conversation flow naturally based on their answers. "
        "Here is a good path for the conversation:\n"
        "1. Start by asking for their name.\n"
        "2. Ask them how they feel about school (if they like it, if it's fun, etc.).\n"
        "3. Based on their answer, ask about their favorite subject or teacher.\n"
        "4. Ask about their friends or what they do during break time.\n\n"
        "IMPORTANT RULES:\n"
        "- Act curious and excited, like a real classmate.\n"
        "- Always respond to what the child says before asking a new question.\n"
        "- If they say something sad or negative (like 'I don't like school'), be empathetic. Say something like, 'Oh no, why not?' before moving on."
    ),
    "store": ( "You are Genie, an AI English tutor in a roleplay with a child at a 'store'. Your goal is to be a friendly shopkeeper. Start by greeting them and asking what they want to buy. React to their choice, then tell them a pretend price to complete the interaction. Keep your replies short, cheerful, and relevant."),
    "home": ("You are Genie, an AI English tutor, in a roleplay with a child about being at 'home'. Your goal is to be a kind and curious family member. Start by asking who they live with. Then, based on their response, ask them what their favorite thing to do at home is. Keep the conversation warm, natural, and encouraging. Ask one question at a time.")
}

# --- Helper Functions ---
def get_gemini_free_chat_response(user_transcript):
    model = get_gemini_model()
    if not model: return "My AI brain isn't working right now. Please check the server logs."
    prompt = (
    ...
    try:
        ...
        return "I'm sorry, I can't talk about that topic. Let's discuss something else!"
    except Exception as e:
        return f"An error occurred: {e}"

def translate_text(text, target_language_code):
    model = get_gemini_model()
    if target_language_code.startswith('en') or not model:
        return text
    ...
    try:
        ...
        if not user_text.strip():
            ...
            if mode == 'roleplay' and scenario in ROLEPLAY_CONTEXTS:
                ...
                chat_session = model.start_chat(history=conversation_history[client_sid]['history'])
                ...
                if client_sid in conversation_history: del conversation_history[client_sid]
        ...
        if '(' in text_for_speech: text_for_speech = text_for_speech.split('(', 1)[0].strip()
